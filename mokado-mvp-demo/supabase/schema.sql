...
-- Café settings (barista PIN hashed)
...
-- Convenience view: balances per user/cafe
...
-- Helper function: current user's balance at a cafe
...
-- Award point with PIN + rate limit (3 minutes)
...
begin
  ...
  -- rate limit: no more than 1 award per user per cafe per 3 minutes
...
-- Redeem a reward if balance >= threshold
...
-- Profiles policies: users can see/update their own
create policy "Profiles are viewable by owner" on public.profiles
for select using (auth.uid() = id);
create policy "Profiles can be updated by owner" on public.profiles
for update using (auth.uid() = id);

-- Cafes: owners can see and manage their own
create policy "Cafes selectable by owner" on public.cafes
for select using (exists (select 1 from profiles p where p.id = auth.uid() and p.id = owner_id));
create policy "Cafes insert by owner" on public.cafes
for insert with check (exists (select 1 from profiles p where p.id = auth.uid() and p.id = owner_id));
create policy "Cafes update by owner" on public.cafes
for update using (exists (select 1 from profiles p where p.id = auth.uid() and p.id = owner_id));

-- Cafe settings: only the owner of the cafe
...
-- Transactions: users see their own; owners see within their cafes
create policy "Transactions: user can read own" on public.transactions
for select using (auth.uid() = user_id);
create policy "Transactions: owner can read cafe data" on public.transactions
for select using (exists (select 1 from cafes c where c.id = cafe_id and c.owner_id = auth.uid()));
create policy "Transactions: insert by function only" on public.transactions
for insert with check (false);

-- Redemptions: similar
create policy "Redemptions: user can read own" on public.redemptions
for select using (auth.uid() = user_id);
create policy "Redemptions: owner can read cafe data" on public.redemptions
for select using (exists (select 1 from cafes c where c.id = cafe_id and c.owner_id = auth.uid()));
create policy "Redemptions: insert by function only" on public.redemptions
for insert with check (false);

-- Profile trigger to create row on signup
