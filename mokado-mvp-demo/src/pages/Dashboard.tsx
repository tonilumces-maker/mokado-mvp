...
export default function Dashboard() {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setProfile(p as any)
      const { data: c } = await supabase.from('cafes').select('id,name').order('name', { ascending: true })
      setCafes(c || [])
    })()
  }, [])
...
function Balances({ cafes }: { cafes: {id:string;name:string}[] }) {
  ...
  async function load() {
    if (!selected) return
    const { data: cs } = await supabase.from('cafe_settings').select('redeem_threshold').eq('cafe_id', selected).single()
    setThreshold(cs?.redeem_threshold ?? null)
    const { data, error } = await supabase.rpc('balance_for', { cafe: selected })
    ...
    <div>
      ...
      {threshold !== null && balance !== null && balance >= threshold && (
        <button className="button" onClick={async () => {
          const { data, error } = await supabase.rpc('redeem_reward', { cafe: selected })
